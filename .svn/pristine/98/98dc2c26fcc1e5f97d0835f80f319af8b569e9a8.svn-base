...
public class QALibFragment extends BaseFragment implements View.OnClickListener {
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        if (isVisibleToUser ) {
            ...
            if (inServiceWXUser != null && PluginHandler.cookie.isPluginLogin()) {
                getQacount("all");
            } else {
                commonqaCount = 0;
                snsCount = 0;
    ...
    private void getQacount(String question_type) {
        if (TextUtils.isEmpty(PluginHandler.cookie.getSign()) || TextUtils.isEmpty(PluginHandler.cookie.getTenantID()))
            return;
